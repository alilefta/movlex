...
import { fetchSimilarTvs } from "../assets/utils/FetchingResources/fetchSimilarTvs";
import { fetchTVCredits } from "../assets/utils/FetchingResources/fetchTVCredits";
import { fetchTVDetails } from "../assets/utils/FetchingResources/fetchTVDetails";
...
const Movie = () => {
	...
	const [isLoadingSimilarTvs, setisLoadingSimilarTvs] = useState(true);
	...
	useEffect(() => {
		fetchSimilarTvs(id)
			.then((data) => {
				setSimilarTvs(data.results.slice(0, 9));
				setisLoadingSimilarTvs(false);
			})
			.catch((error) => {
				console.log(error);
			});
						...
						<nav className="breadcrumb-section" aria-label="breadcrumb">
							<ol className="breadcrumb">
								...
								<li className="breadcrumb-item active" aria-current="page">
									{(mediaDetails && mediaDetails.name) || mediaDetails.title}
								</li>
							</ol>
						</nav>
					</div>
					...
					<div className="may-also-like col-sm-4">
						...
						{isLoadingSimilarTvs ? (
							...
							<div className="similar-media-section">
								<CardsSection media={similarTvs} mediaType={"movie"} />
							</div>
						)}
					</div>
				</div>
